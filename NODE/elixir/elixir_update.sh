...
sudo docker run -d --env-file "$HOME/$container_name/${container_name}.env" --name "$container_name" --platform linux/amd64 --restart always -p "$node_port:$node_port" elixirprotocol/validator
